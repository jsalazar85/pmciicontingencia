angular
    ...
    .controller('tableroSeguimientoDetalleGlobalReportCtrl', [
        ...
        'uiGridConstants',
        '$http',
        tableroSeguimientoDetalleGlobalReportCtrl
    ]);

function tableroSeguimientoDetalleGlobalReportCtrl ($scope,$rootScope,$state,$stateParams,dc,gc,ngProgressFactory,dss,cs,$interval,tools,cds,uiGridConstants,$http)  {
...
*/
    ...
    this.init=function() {
        ...
        console.log('entrando REPORTE DETALLE.......');
        //cds.doWorkTask('grid3Medidas');

        console.log(window.currentMaster);

        if(window.currentMaster){
            $scope.currentMaster=window.currentMaster;
            $scope.getDetalleTabla1Data(window.currentMaster.idIniciativa);
        }
